...
func TestBuildTagSuffix(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			f:    parse.Field{Omit: true, IsChar: false, Slice: false},
			want: ",omitempty",
		},
		{
			name: "required element → no suffix",
			f:    parse.Field{Omit: false},
			want: "",
		},
		{
			name: "optional slice → no omitempty (slice)",
			f:    parse.Field{Omit: true, Slice: true},
			want: "",
		},
		{
			name: "chardata → no omitempty",
			f:    parse.Field{Omit: true, IsChar: true},
			want: "",
		},
		{
			name: "optional attr → omitempty",
			f:    parse.Field{Omit: true, IsAttr: true},
			want: ",omitempty",
		},
	}
...
func TestGenerate_skipAbstract(t *testing.T) {
	types := []*parse.ComplexType{
		{Name: "AbstractBase", Abstract: true, Fields: []parse.Field{}},
		{Name: "ConcreteType", Fields: []parse.Field{
...
func TestGenerate_emptyName(t *testing.T) {
	...
	types := []*parse.ComplexType{
		{Name: "", Fields: []parse.Field{{GoName: "X", GoType: "string", XMLTag: "x"}}},
		{Name: "Valid", Fields: []parse.Field{{GoName: "Y", GoType: "int", XMLTag: "y"}}},
	}
