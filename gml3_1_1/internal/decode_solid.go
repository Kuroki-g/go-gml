...
// handleMultiSolid decodes a gml:MultiSolid and returns a core.Solid
// with all member exteriors merged.
func (r *Reader) handleMultiSolid(dec *xml.Decoder, se xml.StartElement) (core.Geometry, error) {
	...
	if x.SolidMembers != nil {
		for i := range x.SolidMembers.Solid {
			...
			solid.Exterior = append(solid.Exterior, s.Exterior...)
			solid.Interior = append(solid.Interior, s.Interior...)
		}
