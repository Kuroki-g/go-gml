...
// Covered code: gml3_2_1/internal/decode_linestring.go (preferDim + field-count inference block).
func TestDecode_LineString_Pos(t *testing.T) {
	const ns = `xmlns:gml="http://www.opengis.net/gml/3.2"`
	tests := []struct {
		name    string
		src     string
		wantPts int
...
// Covered code: gml3_2_1/internal/decode_linestring.go (coordinates branch).
...
// in LinearRing. Covered code: gml3_2_1/internal/decode_polygon.go (ringFromLinearRing).
...
// This is the PLATEAU CityGML 2.0 pattern: srsDimension="3" only on root Envelope.
func TestNext_GlobalDim_PropagatedToPolygon(t *testing.T) {
	const src = `<root xmlns:gml="http://www.opengis.net/gml/3.2">
