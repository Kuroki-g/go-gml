...
func preScanGeometries(dec *xml.Decoder, resolver *curveResolver) error {
	...
	for {
		...
		case gmlSurface:
			...
			if x.Patches != nil {
				if mp, err := multiPolygonFromSurfacePatchArrayProperty(x.Patches, preferDim(x.SrsDimension, nil), x.SrsName, resolver); err == nil {
					if len(mp) == 1 {
		...
		case gmlTin:
			...
			if mp, err := multiPolygonFromSurfacePatchArrayProperty(x.TrianglePatches, nil, nil, resolver); err == nil {
				resolver.multiPolygonByID[id] = mp
			}
		case gmlTriangulatedSurface:
			...
			if mp, err := multiPolygonFromSurfacePatchArrayProperty(x.Patches, nil, nil, resolver); err == nil {
				resolver.multiPolygonByID[id] = mp
			}
